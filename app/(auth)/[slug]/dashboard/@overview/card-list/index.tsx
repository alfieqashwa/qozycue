...
export function CardList({ date, setDate }: Props) {
  return (
    <>
      <TotalRevenue date={date} setDate={setDate} />
    </>
  )
}

export function TotalRevenue({ date, setDate }: Props) {
  // const totalRevenue = await api.order._sumRevenue()
  ...
  return (
    <ToggleTotalRevenue
      revenueIncludeTaxes={revenueIncludeTaxes}
      revenueExcludeTaxes={revenueExcludeTaxes}
      totalTransactions={totalTransactions ?? 0}
    />
  )
}

type ToggleTotalRevenueProps = {
  revenueIncludeTaxes: string
  revenueExcludeTaxes: string
  totalTransactions: number
}

export function ToggleTotalRevenue({
  revenueIncludeTaxes,
  revenueExcludeTaxes,
  totalTransactions,
}: ToggleTotalRevenueProps) {
