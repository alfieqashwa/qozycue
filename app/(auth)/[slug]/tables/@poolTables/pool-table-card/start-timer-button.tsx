...
export function StartTimerButton({
  ...
  const packets = api.packet.findAllByCompanyId.useQuery(undefined, {
    select(data) {
      return data
        .filter((d) => d.status === Status.enabled)
  ...
  const isHourly =
    packets.data?.find((p) => p.id === packetIdWatch)?.rate === "HOUR"

  const handleOnCloseAutoFocus = () => {
    form.reset()
  }

  const { mutate, isPending } = api.poolRental.startTimer.useMutation({
    async onSuccess() {
      toast({
        title: "Succeed!",
        variant: "default",
        description: <p>Table {poolTableName} has been started.</p>,
      })
  ...
  function onSubmit(values: TStartTimer) {
    ...
    const costCalc = packets.data?.find((p) => p.id === packetId)
      ?.cost as number
    const rate = packets.data?.find((p) => p.id === packetId)?.rate as Rate
