...
export const protectedProcedure = async (ctx: QueryCtx) => {
  const userId = await getAuthUserId(ctx)
...
export const reset = async (
  ctx: MutationCtx,
  { forReal }: { forReal: string },
) => {
  if (forReal !== "reset-batman") {
...
export const createTrialCompany = async (
  ctx: MutationCtx,
  { userId, companyId }: { userId: Id<"users">; companyId: Id<"companies"> },
) => {
...
export const subscriptions = async (
  ctx: QueryCtx,
  { companyId }: { companyId?: Id<"companies"> },
) => {
...
export function validateSubscriptionLimits({
  ...
  const limits = {
    ...
    user: { TRIAL: 5, BASIC: 7, PRO: 10, ENTERPRISE: undefined },
  }
